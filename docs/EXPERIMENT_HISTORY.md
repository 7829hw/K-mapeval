...
This document records observations at particular revisions and experimental conditions.
Historical recommendations and present-tense statements below describe their original context;
they are not current task instructions. Follow [AGENTS.md](../AGENTS.md) for current rules and
[REFERENCE_MAPPING.md](REFERENCE_MAPPING.md) for the current upstream mapping.

Migrated from AGENTS.md and REFERENCE_MAPPING.md on 2026-09-05. Measurements, revision IDs,
dataset names, and historical caveats are preserved, not re-measured. Original reports under
`reports/` remain the evidence artifacts. Paths written in backticks are repository-relative
unless explicitly absolute. Overlapping accounts are retained where they carry distinct context.

Known inconsistencies in the source accounts, not resolved by this document move:

- The v9 AGENTS account compares a later mean against 71.2, but also lists a pre-change mean
  of 70.4. Verify report provenance before using either as a comparison baseline.
- Blanket claims that every dataset is spent, or that one set is the only holdout, were
  time-dependent. Held-out status belongs to a dataset/revision and its use in development.
- Shared-budget recommendations and claims that all v7 runs used today's settings are
  historical. The current configuration is ReAct 15 / Spatial-Agent 30, concurrency 32.
- The old checklist's claim that intent still reaches retrieval/evaluation predates its removal.

## Reading guide

- [Detailed experiment narrative](#detailed-experiment-narrative): original reference-document
  chronology, including upstream measurements and superseded implementations.
- [Dataset summaries formerly in AGENTS.md](#dataset-summaries-formerly-in-agentsmd): compact
  benchmark accounts, with the original per-family caveats.
- [Evidence formerly embedded in instructions](#evidence-formerly-embedded-in-instructions):
  measurement rationale formerly mixed into operational rules.

## Detailed experiment narrative

The upstream context-cache implementation referenced below is described in the
[current mapping](REFERENCE_MAPPING.md#context-evidence-upstreams-local-context-cache).
Historical mentions of benchmark history in `docs/REFERENCE_MAPPING.md` now refer to this
document; original wording is retained to preserve provenance.

The former mapping table described the v2 dataset, not the current standard builder:

| Upstream concept | Historical implementation | Historical deviation |
|---|---|---|
...
## Dataset summaries formerly in AGENTS.md
...
## Evidence formerly embedded in instructions
