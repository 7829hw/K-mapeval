...
|---|---|---|
| MapEval `Evaluator2.py` structured ReAct loop | `agents/react/agent.py` | Removes the localhost backend, 30/60-second sleeps, and remote result writes. Keeps 1-based multiple choice and `^^N^^`. |
| MapEval `FormattedTools.py` | `tools/registry.py` | Thin tools return canonical JSON rather than provider-specific prose. No embedded bearer token. |
| MapEval `Tools.py` / MapQaTor backend | `providers/base.py`, `providers/kakao.py` | Provider dependency injection replaces the separate HTTP backend. |
...
| Spatial-Agent operators | `tools/spatial_ops.py` | Reimplements semantically stable distance, bearing, sort, min/max and route aggregation without Google-specific fields. |
