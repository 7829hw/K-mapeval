...
- nearby_places(center, query|category_code, radius_m, limit) -> [Place]
...
- bearing_to_direction(place_a, place_b) -> {bearing_degrees,direction}
...
numbering is 1-based. Return JSON only:
{"predicted_option":2,"confidence":0.8,"reason":"brief evidence-based reason"}
...
class SpatialAgent(BenchmarkAgent):
    ...
    def answer(self, question: str, options: list[str]) -> AgentResult:
        ...
        try:
            ...
            if intent not in {"nearby", "poi", "routing", "trip"}:
        ...
        if predicted is None and failure_type is None:
            ...
            if provider_failure:
                ...
                failure_message = "No valid 1-based option found during evaluation"
        return AgentResult(
...
def _coerce_option(value: Any, option_count: int) -> int | None:
    try:
        ...
        return None
    return option if 1 <= option <= option_count else None
...
def _heuristic_intent(question: str) -> str:
    ...
    if any(word in lowered for word in ("가까운", "근처", "nearest", "nearby")):
        return "nearby"
    if any(word in lowered for word in ("경로", "운전", "거리", "route", "driving")):
        return "routing"
