...
from ..tools.kakao_maps import KakaoMapsClient
...
class TransformationExecutor:
    ...
    def __init__(self, kakao_client: KakaoMapsClient):
    ...
    def _execute_operator(
        ...
        if operator_name == 'query_local_place':
            ...
            if place_name:
                ...
                #   Kakao Map
                if result is None:
                    logger.info(f"[query_local_place]  '{place_name}', Kakao Map")
                    try:
                        from src.spatial_agent.agent.operators import geocode as geocode_op, place_details as place_details_op
                        ...
                        if geocoded and geocoded.get('place_id'):
                            ...
                            if details:
                                # 
                                details['from_kakao_fallback'] = True
                                details['local_db_name'] = place_name
                                logger.info(f"[query_local_place] OK Kakao : {place_name}")
                                return details
                    except Exception as e:
                        logger.warning(f"[query_local_place] Kakao : {e}")
            ...
            if center and place_type:
                ...
                # , Kakao Map place_search
                if not results:
                    logger.info(f"[query_local_nearby_places] , place_search API")
                    try:
                        # 
                        from src.spatial_agent.agent.operators import geocode as geocode_op
                        center_loc = geocode_op(self.client, center)
                        if center_loc:
                            #  place_search
                            from src.spatial_agent.agent.operators import place_search as place_search_op
        ...
        if operator_name == 'geocode':
            ...
            # 
            from src.spatial_agent.agent.operators import filter_places_by_time as filter_by_time_func
            ...
            if (lat1 is None or lng1 is None) and location_a_id and isinstance(location_a_id, str):
                ...
                from src.spatial_agent.agent.operators import query_local_coordinates
            ...
            if locations and all(isinstance(loc, str) for loc in locations):
                try:
                    from src.spatial_agent.agent.operators import query_local_places_batch
