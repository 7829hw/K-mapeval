...
class APIProcessor:
    ...
    def nearby_search(self, state: SpatialAgentState) -> SpatialAgentState:
        ...
        results = self.kakao_client.nearby_search(
    ...
    def get_directions(self, state: SpatialAgentState) -> SpatialAgentState:
        ...
        directions = self.kakao_client.get_directions(
            origin=origin_str,
    ...
    def get_place_details(self, state: SpatialAgentState) -> SpatialAgentState:
        ...
        for location in locations:
            ...
            if location.place_id:
                ...
                details = self.kakao_client.get_place_details(location.place_id)

            if not details:
                self.logger.info("Fallback to text search for location: %s", location.name)
                search_results = self.kakao_client.text_search(location.name, location=search_location, radius=20000)
                if search_results:
                    candidate = search_results[0]
                    place_id = candidate.get("place_id")
                    if place_id:
                        details = self.kakao_client.get_place_details(place_id)
    ...
    def calculate_distances(self, state: SpatialAgentState) -> SpatialAgentState:
        ...
        matrix = self.kakao_client.get_distance_matrix(
