...
##  (2 )
- _extract_metric_value:  Kakao Map API 
...
def query_local_place(place_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        from src.spatial_agent.tools.local_context_db import ContextManager
...
def query_local_coordinates(place_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        from src.spatial_agent.tools.kakao_maps import KakaoMapsClient
...
def geocode(
    ...
    #  Kakao Map API with location bias and region
...
def reverse_geocode(client: KakaoMapsClient, lat: float, lng: float) -> Optional[str]:
    """
    : 
...
def place_details(client: KakaoMapsClient, place_id: str) -> Optional[Dict[str, Any]]:
    """
                ...
                if not waypoints_verified:
                    log_highlight_event(
                        ...
                        details=[
                            ...
                            "Waypoints may have been ignored by Kakao Map API",
...
def open_at_time(place_detail: Dict[str, Any], local_dt: datetime) -> Optional[bool]:
    ...
    # Kakao Places API  weekday: 0=, 1=, ..., 6=
