...
class SpatialAgent:
    ...
    def __init__(
        ...
        # API
        self.kakao_client = KakaoMapsClient(kakao_api_key)
        self.llm = ChatOpenAI(
        ...
        try:
            ...
            if db_path.exists():
                ...
                LOGGER.warning(f"[GLOBAL_DB] : {db_path}, Kakao Map API")
        except Exception as e:
            LOGGER.warning(f"[GLOBAL_DB] : {e}, Kakao Map API")

        # ()
        self.agent_routing = AgentRouting(self.llm)  # 
        self.planner_agent = PlannerAgent(self.llm)  #  + DAG
        self.transformation_executor = TransformationExecutor(self.kakao_client)
