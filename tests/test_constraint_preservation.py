...
The measurement that produced this file: requiring a `FILTER` node was violated by 45 of 47
recorded `Search-Narrow-Rank` rows and 37 of those answered correctly, because the kind rides on
`nearest` as readily as on a filter. Presence of a transformation is not enforcement of a
constraint, so what is checked is the constraint: some step on the way to the Measure carries it.

Three pieces hold it up. The vocabulary distinguishes measuring a *set* from measuring a pair, so
the planner says which it means instead of the factorizer inferring it from how many inputs
arrived. A restriction is a concept, and the node applying it names that concept as an input. And
after grounding, a restriction the question states that nothing carries is a refusal -- because
the alternative is a clean run that answered a question with the condition left off.
"""
...
def test_an_input_that_names_nothing_at_all_is_still_refused() -> None:
...
def test_a_pairwise_graph_that_cannot_hold_the_stated_kind_is_refused() -> None:
    ...
    kind, on a question that asked for the nearest 중식 one.
    """
...
def test_a_carrier_that_never_reaches_the_measure_does_not_count() -> None:
    ...
    assert len(_unpreserved_constraints(grounded, FACTS)) == 1
