...
def test_distance_answers_are_graded_with_a_tolerance_but_the_strict_number_survives(
    tmp_path,
) -> None:
    class OffByOneAgent(BenchmarkAgent):
        agent_type = "off-by-one"

        def answer(self, question: str, options: list[str]) -> AgentResult:
            return AgentResult(agent_type=self.agent_type, predicted_answer=1)
    ...
    verdicts = {row["id"]: (row["answer_correct"], row["strict_answer_correct"])
                for row in report.results}
    assert verdicts["near"] == (True, False)  # 12 m out, inside the tolerance
    assert verdicts["far"] == (False, False)  # 300 m out
    assert verdicts["other"] == (False, False)  # tolerance is distance-only
    assert report.statistics["overall_answer_accuracy"]["correct"] == 1
    assert report.statistics["strict_answer_accuracy"]["correct"] == 0
    assert report.metadata["distance_tolerance_m"] == 20.0


def test_a_zero_tolerance_grades_distance_answers_exactly(tmp_path) -> None:
    class OffByOneAgent(BenchmarkAgent):
        agent_type = "off-by-one"
    ...
    report = Evaluator(
        OffByOneAgent(), items, output_dir=None, log_dir=tmp_path / "logs"
    ).run()
